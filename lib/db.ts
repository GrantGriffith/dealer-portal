...
export async function getAllDealers(): Promise<(Dealer & { manufacturer_count: number })[]> {
  const { rows } = await sql<Dealer & { manufacturer_count: number }>`
...
export async function getDealerById(id: number): Promise<Dealer | null> {
  const { rows } = await sql<Dealer>`SELECT * FROM dealers WHERE id = ${id} LIMIT 1`
  return rows[0] ?? null
}
...
export async function updateDealer(
  ...
  // Fetch current values then merge — avoids complex conditional SQL
...
export async function getAllManufacturers(): Promise<Manufacturer[]> {
  const { rows } = await sql<Manufacturer>`
    SELECT * FROM manufacturers ORDER BY category, name
  `
  return rows
}

export async function getManufacturerById(id: number): Promise<Manufacturer | null> {
  const { rows } = await sql<Manufacturer>`SELECT * FROM manufacturers WHERE id = ${id} LIMIT 1`
  return rows[0] ?? null
}
...
export async function updateManufacturer(
  ...
  const { rows } = await sql<Manufacturer>`
    ...
    SET name = ${name}, category = ${category}, logo_url = ${logoUrl}, price_list_url = ${plUrl}
    WHERE id = ${id}
    RETURNING *
  `
  return rows[0] ?? null
